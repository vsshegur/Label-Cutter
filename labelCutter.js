...
document.getElementById('lc_logoInput').addEventListener('change', (e) => {
  ...
  r.onload = (ev) => {
    const img = new Image(); img.onload = () => {
      ...
      lc_customLogoBase64 = cvs.toDataURL('image/png'); 
      document.getElementById('lc_logoPreview').src = lc_customLogoBase64; 
...
document.getElementById('lc_pdfFileInput').addEventListener('change', e => {
  ...
  files.forEach(f => lc_rawFiles.push({ id: Math.random().toString(36).substr(2), name: f.name, file: f })); 
  lc_updateUI(); 
...
function lc_groupLines(items) {
  const pts = items.filter(it => it.str && it.str.trim().length > 0).map(it => ({ 
      ...
      height: it.height || Math.abs(it.transform[3]) || 9 
  }));
...
async function lc_extract(page, platform) {
  ...
  if (platform === 'flipkart') {
      ...
      for (let i = 0; i < words.length; i++) {
          if (/IMEI\/SrNo/i.test(words[i])) {
              ...
              for (let j = i - 1; j >= Math.max(0, i - 25); j--) {
                  ...
                  if (!w || /^[\-\+]?[0-9,]+(\.[0-9]+)?%?$/.test(w) || /^x$/i.test(w)) continue;
      ...
      if (fkSkus.length > 0) { 
          let uniqueSkus = [...new Set(fkSkus)]; 
          sku = uniqueSkus.join(' + '); 
          qty = totalQtyVal !== null ? totalQtyVal : fkSkus.length;
      } else { 
      ...
      if (detailsMatch) {
          let block = detailsMatch[1].replace(/SKU|Size|Qty|Color|Order No\.?|Free Size|Multicolor/ig, '').replace(/\|/g, ' ').trim(); 
...
async function lc_generatePdf() {
  ...
  for (let i = 0; i < sortedData.length; i++) {
      ...
      if (is4x4Combined) {
          ...
          const bannerH = 95; const contentH = printableH - bannerH - margin;
          const embBox = { left: item.fullBox.left, bottom: item.fullBox.bottom, right: item.fullBox.right, top: item.fullBox.top }; 
          const embedded = await outDoc.embedPage(srcPage, embBox);
          const eW = embBox.right - embBox.left; const eH = embBox.top - embBox.bottom; 
          ...
          if (currentPlatform === 'meesho' && logo && item.msPos) { 
              ...
              p1.drawRectangle({ x: lx-2, y: ly-2, width: lw+4, height: lh+4, color: rgb(1,1,1) }); 
              p1.drawImage(logo, { x: lx, y: ly, width: lw, height: lh }); 
          }
          if (incInv && item.iBox) {
...
document.getElementById('lc_downloadBtn').addEventListener('click', async () => {
  try { 
      ...
      a.download = `Shegurs_${currentPlatform.toUpperCase()}_${sizeName}_${new Date().toISOString().slice(0,10)}.pdf`; 
      a.click(); 
...
function lc_loadRecoveredPdfs() {
    ...
    req.onsuccess = () => {
        ...
        req.result.forEach(item => { 
            ...
            if (ageHours >= 6) tx.objectStore('pdfs').delete(item.id); 
...
window.lc_handleRecovery = function(id, action) {
    ...
    req.onsuccess = () => { 
        ...
        if (action === 'download') { 
            const a = document.createElement('a'); 
            a.href = url; 
            a.download = `Shegurs_${req.result.platform.toUpperCase()}_Recovered_${id}.pdf`; 
            a.click(); 
            URL.revokeObjectURL(url); 

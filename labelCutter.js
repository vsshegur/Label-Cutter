...
let lc_rawFiles = []; let lc_parsedData = []; let currentPlatform = 'flipkart'; let lc_customLogoBase64 = null;
...
window.addEventListener('appUnlocked', () => {
    ...
    } catch(e) { document.getElementById('lc_logoPreview').src = DEFAULT_BRAND_LOGO; }
...
document.getElementById('lc_processBtn').addEventListener('click', async () => {
  ...
  } catch (err) { console.error(err); alert("Error processing PDF: " + err.message); document.getElementById('loader').classList.add('hidden'); document.getElementById('lc_processBtn').disabled = false; }
});
...
async function lc_generatePdf() {
  ...
  if(window.appState.currentUser && db) {
      ...
      tx.objectStore('pdfs').put({ id: Date.now(), userId: window.appState.currentUser.uid, platform: currentPlatform, totalOrders: lc_parsedData.length, totalPieces: lc_parsedData.reduce((s,i)=>s+i.qty,0), data: pdfBytes, timestamp: Date.now() });

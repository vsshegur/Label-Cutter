...
async function loadAdminUsers() {
    ...
    try {
        ...
        snapshot.forEach(docSnap => {
            ...
            tr.innerHTML = `<td class="py-4 px-4 flex items-center gap-3"><img src="${u.photo || 'https://via.placeholder.com/24'}" class="w-8 h-8 rounded-full shadow-sm"><div><p class="font-bold text-white">${u.name}</p><p class="text-[10px] text-slate-500">${u.email}</p></div></td><td class="py-4 px-4"><span class="px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest ${u.role === 'admin' ? 'bg-indigo-500/20 text-indigo-400' : 'bg-slate-800 text-slate-400'}">${u.role}</span></td><td class="py-4 px-4"><span class="px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest ${u.planType === 'Free Trial' ? 'bg-orange-500/20 text-orange-400' : 'bg-emerald-500/20 text-emerald-400'}">${u.planType}</span></td><td class="py-4 px-4 font-black text-xs ${isExp && u.role !== 'admin' ? 'text-rose-500' : 'text-slate-300'}">${daysLeft} DAYS</td><td class="py-4 px-4 flex justify-end items-center gap-2">${u.role !== 'admin' ? `<button onclick="window.updateDaysAdmin('${uid}')" class="bg-indigo-600 hover:bg-indigo-500 text-white text-[9px] px-3 py-2 rounded uppercase tracking-widest font-black transition-colors">Add 30 Days</button>` : '<span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Sys Admin</span>'}</td>`; tbody.appendChild(tr);
        });
    } catch(e) {}
}
